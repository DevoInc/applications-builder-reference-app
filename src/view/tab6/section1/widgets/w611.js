import availabilityTimeline from '@devoinc/applications-builder/widgets/availabilityTimeline';
...
export default function buildw611() {
  let widget = availabilityTimeline('w611');
  widget.setRequests([requests.get('mapTime')]);

  widget.setKey('clientIpAddress');
  widget.setValue('count');
  widget.setGrouping(60 * 60 * 1000);
  ...
  var setDateFormatter = new CheckBoxField({
    ...
    onChange: (val) =>
      widget.setDateFormatter(val ? 'YYYY-MM-DD HH:mm:SS' : ''),
  });
  ...
  var setTooltipHasDataClass = new CheckBoxField({
    ...
    onChange: (val) =>
      widget.setTooltipHasDataClass(val ? 'tultiphaydatos' : ''),
  });

  var setTooltipHasNoDataClass = new CheckBoxField({
    id: '#w612-setTooltipHasNoDataClass',
    onChange: (val) =>
      widget.setTooltipHasNoDataClass(val ? 'tultipnohaydatos' : ''),
  });
  ...
  var setSubtitleDateFormat = new CheckBoxField({
    ...
    onChange: (val) =>
      widget.setSubtitleDateFormat(val ? 'YYYY-MM-DD HH:mm:SS' : ''),
  });
